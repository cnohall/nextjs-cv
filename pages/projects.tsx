...
export default function Projects() {
  ...
  return (
      <div className="py-12">
        <Head>
          <title>{t('common:projects_title')}{t('common:page_title_separator')}</title>
        </Head>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div key={index} className="group bg-white/5 border border-white/10 p-8 rounded-2xl hover:border-green-500/50 transition-all flex flex-col">
              ...
              <p className="text-silver mb-6 flex-grow leading-relaxed">
                {project.description}
              </p>
              <div className="flex flex-wrap gap-2 mb-8">
                {project.tags?.map(tag => (
                  <span key={tag} className="text-[10px] uppercase tracking-widest font-bold px-3 py-1 bg-white/5 rounded-full border border-white/10 text-silver">
                    {tag}
                  </span>
                ))}
              </div>
